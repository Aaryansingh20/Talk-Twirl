...
//import LogoFeatureModal from "@/components/ui/videocallbtn"
...
export default function Sidebar({ preloadedUserInfo,preloadedConversations}: SideBarProps) {
  ...
  return (
    <div className="w-[70px] md:w-[380px] lg:w-1/4 h-screen flex flex-col bg-background bg-black  border-border border-gray-800 border-r-2">
      {/* Header */}
      <div className="shrink-0 px-3 py-[18px] md:py-[14px] bg-black flex justify-center md:justify-between items-center">
        <Link href="/profile">
          <Avatar>
            <AvatarImage className="w-8 h-8 md:w-9 md:h-9 lg:w-10 lg:h-10 rounded-full" src={userInfo?.profileImage} alt="Your avatar" />
          </Avatar>
        </Link>
        ...
        {filteredConversations?.map((chat) => (
          <Link href={`/chat/${chat.id}`} key={chat.id}>
            <div className={`flex items-center px-2 py-2 md:px-3 md:py-3 hover:bg-[#202C33] cursor-pointer
                  ${pathname.split("/")?.[2] === chat?.id ? "bg-[#1d1e1e]" : ""}
                `}>
              <div className="relative">
                <Avatar>
                  <AvatarImage className="w-8 h-8 lg:w-10 lg:h-10 rounded-full" src={chat?.chatImage} />
    ...
    <>
      {parts.map((part, i) => (
        part.toLowerCase() === searchQuery.toLowerCase() ?
          <span key={i} className="bg-blue-600 text-white px-0.5 rounded">
            {part}
          </span>
          :
